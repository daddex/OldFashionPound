package org.daddex.oldfashionpound.business;

import org.daddex.oldfashionpound.exception.AmountExceededException;
import org.daddex.oldfashionpound.exception.BadDivisionInputException;
import org.daddex.oldfashionpound.exception.BadInputException;
import org.daddex.oldfashionpound.exception.BadMultiplyInputException;
import org.daddex.oldfashionpound.model.bean.OldUkCurrency;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

import static org.daddex.oldfashionpound.utils.AppConstant.*;
import static org.daddex.oldfashionpound.utils.TranscoderUtil.*;
import static org.daddex.oldfashionpound.utils.TranscoderUtil.formatNumberToCurrency;

public class OperationDivision implements  IOperationStrategy{

    private Pattern regexPattern;
    public OperationDivision(Pattern currencyInputParser) {
        regexPattern=currencyInputParser;
    }
    @Override
    public String doOperation(String element) {
        Matcher matcher = regexPattern.matcher(element);
        if (matcher.find()) {
            Integer pounds_A =removeCharacterFromImport(matcher.group(1));
            Integer shillings_A = removeCharacterFromImport(matcher.group(2));
            Integer pence_A =  removeCharacterFromImport(matcher.group(3));
            Integer divider =removeCharacterFromImport(matcher.group(4));
            if(divider == 0){
                throw new BadDivisionInputException(DIVIDER_INPUT_ERROR_MESSAGE,element);
            }
            OldUkCurrency oldUkCurrency_A = OldUkCurrency.builder().pounds(pounds_A).shillings(shillings_A).pennies(pence_A).build();

            String result  =  computeDivision(oldUkCurrency_A,divider);
            System.out.println(String.format(RESULT_DEBUG_MESSAGE," division ",result));
            return  result;
        }
        throw new BadDivisionInputException(DIVISION_INPUT_ERROR_MESSAGE,element);
    }
    private String computeDivision(OldUkCurrency oldUkCurrency_A,Integer divider){
        Integer pennies_A = convertInputToPennies(oldUkCurrency_A);
        Integer product = Math.floorDiv(pennies_A,divider);

        OldUkCurrency oldUkCurrencyResult = convertPenniesToOutput(product);
        oldUkCurrencyResult.setRemainder(Math.floorMod(pennies_A,divider));
        return formatNumberToCurrency(oldUkCurrencyResult);
    }
}
